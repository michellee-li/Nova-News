...
export default function UserScreen() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (userErr || !userData?.session?.access_token) {
        setLoading(false);
      ...
      const resp = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/api/delete-account`, {
        method: "POST",
        headers: {
      ...
      // Navigate to your disguised landing / NewsList
      // @ts-ignore
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F7F2EA" }}>
      ...
      <View style={{ paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {/* ESCAPE (optional keep consistent UI) */}
        <TouchableOpacity
          onPress={() => {
            // @ts-ignore
            nav.navigate("NewsList");
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 8 }}>
        ...
        <Section title="Data & Privacy">
          ...
          <View style={{ backgroundColor: "white", padding: 16, borderBottomLeftRadius: 12, borderBottomRightRadius: 12, borderTopWidth: 1, borderColor: "#EEE" }}>
            <Text style={{ fontSize: 16, fontWeight: "600", color: "#b00020", marginBottom: 6 }}>Delete My Account</Text>
            <Text style={{ color: "#5F6368", marginBottom: 10 }}>Permanently delete your account and all associated data. This action cannot be undone.</Text>
            <TouchableOpacity
              onPress={() => setOpen(true)}
              style={{ alignSelf: "flex-start", backgroundColor: "#FFE7E7", borderColor: "#FFB3B3", borderWidth: 1, paddingVertical: 10, paddingHorizontal: 14, borderRadius: 10 }}
            >
        ...
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.35)", justifyContent: "flex-end" }}>
          <View style={{ backgroundColor: "white", padding: 16, borderTopLeftRadius: 16, borderTopRightRadius: 16 }}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "flex-end", marginTop: 14 }}>
              <TouchableOpacity onPress={() => { setOpen(false); setConfirm(""); setAck(false); }} style={{ padding: 12 }}>
                <Text>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
...
function Section({ title, children }: any) {
  return (
    <View style={{ marginBottom: 18 }}>
      <Text style={{ marginLeft: 16, marginBottom: 8, color: "#5F6368", fontSize: 12, fontWeight: "700" }}>{title.toUpperCase()}</Text>
...
function Row({ label, onPress }: any) {
  return (
    <TouchableOpacity onPress={onPress} style={{ backgroundColor: "white", padding: 16, borderTopWidth: 1, borderColor: "#EEE" }}>
